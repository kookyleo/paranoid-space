...
mod tests {
    ...
    fn test_spacing_html() {
        ...
        let result = spacing_html(script_html);
        assert!(result.contains("<script>"));
        assert!(result.contains("let x = '你好World';"));
        assert!(result.contains("</script>"));

        let style_html = "<style>.class{color:red;}</style>";
        let result = spacing_html(style_html);
        assert!(result.contains("<style>"));
        assert!(result.contains(".class{color:red;}"));
        assert!(result.contains("</style>"));
        ...
        assert_eq!(
            ...
            "<div>文本 Text<a>链接 Link<em>强调 Emphasis</em></a></div>"
        );
