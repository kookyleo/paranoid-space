//! Module level inner doc comment模块级别的内部文档注释
...
/// Outer line doc comment函数外部文档注释
...
fn example_function<T: Display>(item: T) { // Generic function 泛型函数
    ...
    let bytes = b"ASCII only byte字符串"; // Byte string字节串
...
impl<T: Display> Point<T> {
    /// Associated function for Point Point的关联函数
    fn new(x: T, y: T) -> Self {
...
/// Trait定义 Trait Definition
trait Summable {
    /// Method required by trait Trait 要求的方法
    fn sum(&self) -> i32; // Method signature 方法签名
}

/// Trait implementation for Point<i32> 为 Point<i32>实现Trait
